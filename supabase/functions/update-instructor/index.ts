...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { error: instructorUpdateError } = await supabaseAdmin
      .from('instructor_profiles')
      .update({ department: department.trim() })
      .eq('profile_id', profileId)

    if (instructorUpdateError) {
      return new Response(JSON.stringify({ error: instructorUpdateError.message }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
