...
class _ProfessorSubjectScreenState extends State<ProfessorSubjectScreen> {
  ...
  void _showEnrollStudentDialog() {
    ...
    StateSetter? dialogSetS;

    _repo.fetchAllStudents().then((students) {
      allStudents = students.where((s) => !enrolledIds.contains(s['profileId'])).toList();
      results = List.from(allStudents);
      loading = false;
      if (dialogSetS != null) {
        dialogSetS!(() {});
      }
                ...
                children: [
                  Row(
                    children: [
                      Container(
                        ...
                        child: const Icon(Icons.person_add_alt_1_rounded, color: AppColors.authPrimary, size: 24),
                      ),
                      const SizedBox(width: 14),
                      const Text(
                        'Enroll Student',
                  ...
                  TextField(
                    ...
                    onChanged: (val) {
                      ...
                      final filtered = allStudents.where((s) {
                        final num = (s['studentNumber'] ?? '').toLowerCase();
                  ...
                  if (loading)
                          ...
                          return Container(
                            ...
                            child: ListTile(
                              ...
                              leading: CircleAvatar(
                                ...
                                child: Text(s['name']![0], style: const TextStyle(color: AppColors.authPrimary, fontWeight: FontWeight.bold)),
                              ),
                              title: Text(s['name'] ?? '', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                              subtitle: Text('${s['studentNumber']} • ${s['course']} ${s['yearSection']}', style: TextStyle(fontSize: 12, color: Colors.grey.shade600)),
                              trailing: ElevatedButton(
                                onPressed: () async {
                                  try {
                                    await _repo.enrollStudent(widget.subject.id, s['profileId']!);
                                    enrolledIds.add(s['profileId']!);
                                    allStudents.removeWhere((item) => item['profileId'] == s['profileId']);
                                    results.removeWhere((item) => item['profileId'] == s['profileId']);
                                    setS(() {});
                                    await _load();
                                ...
                                child: const Text('Enroll', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
                              ),
                            ),
                          );
                        },
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          Icon(Icons.person_search_rounded, size: 36, color: Colors.grey.shade400),
                          const SizedBox(height: 8),
                          Text('No students found.', style: TextStyle(color: Colors.grey.shade500, fontSize: 13, fontWeight: FontWeight.w500)),
                        ],
                      ),
                    ),
                  const SizedBox(height: 16),
