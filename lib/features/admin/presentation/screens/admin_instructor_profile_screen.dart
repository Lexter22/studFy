...
class _AdminInstructorProfileScreenState
  ...
  void initState() {
    ...
    _seedHandledSubjects();
    _requests = [
      {
        'icon': Icons.person_add,
        'title': widget.initialRequest ?? 'Class Creation',
      },
      {'icon': Icons.access_time_filled, 'title': 'Schedule Conflict Request'},
    ];
  }

  void _seedHandledSubjects() {
    final offerings = context.read<AppState>().subjectOfferings;
    final matches = offerings.where(
      (offering) => offering['professor'] == _currentInstructor.name,
    );
    ...
    _handledSubjects
      ...
      ..addAll(
        matches.map(
          (offering) => _HandledSubject(
            subjectName: offering['name'] ?? '',
            courseSection:
                '${offering['course'] ?? ''} ${offering['section'] ?? ''}'
                    .trim(),
            timeRoom: 'TBA',
          ),
        ),
  ...
  void _saveEdits() {
    setState(() {
      _currentInstructor = Instructor(
        profileId: _currentInstructor.profileId,
  ...
  void _commitAssignment() {
    // ── Validation ─────────────────────────────────────────────────────────
    final bool anyEmpty =
        _courseCodeCtrl.text.trim().isEmpty ||
    ...
    if (anyEmpty) {
      AppDialog.alert(
        ...
        message:
            'All fields must be filled before saving the class assignment.',
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          ..._handledSubjects.asMap().entries.map(
            (entry) => _buildSubjectRowRaw(
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            flex: 2,
            child: Text(col3, textAlign: TextAlign.right, style: style),
          ),
          if (!isHeader)
            SizedBox(
              width: 35,
            ...
            child: _isEditing
                ...
                : Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Course: ${_currentInstructor.course}',
